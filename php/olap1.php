...
{   
  ...
  if($manufacturer_name) {
    $condition .= " and manufacturer.manufacturer_name = :manufacturer_name";
  }
  if($year || $quarter) {
    $from .= ",calender";
  ...
  if(empty($manufacturer_name)) {
    $condition .= " group by manufacturer.manufacturer_name";
  }
  ...
  // Fetch matching row.
  if($year && $quarter && !$manufacturer_name){
    $ps->execute(array(':year' => $year, ':quarter' => $quarter));
  }
  if($year && $manufacturer_name && !$quarter){
    $ps->execute(array(':year' => $year, ':manufacturer_name' => $manufacturer_name));
  }
  if($quarter && $manufacturer_name && !$year){
    $ps->execute(array(':quarter' => $quarter, ':manufacturer_name' => $manufacturer_name));
  }
  if($quarter && $manufacturer_name && $year){
    $ps->execute(array(':quarter' => $quarter, ':manufacturer_name' => $manufacturer_name,':year' => $year));
  }
  if($year && !$manufacturer_name && !$quarter){
    $ps->execute(array(':year' => $year));
  }
  if($quarter && !$year && !$manufacturer_name){
    $ps->execute(array(':quarter' => $quarter));
  }
  if($manufacturer_name && !$quarter && !$year){
    $ps->execute(array(':manufacturer_name' => $manufacturer_name));
  }
  else {
    $ps->execute();
  }
